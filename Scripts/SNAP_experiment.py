...
class SNAP():
    ...
    def find_modes(self,peak_height=2,  min_peak_distance=0.05):
        ...
        mode_indexes,_=find_peaks(T_shrinked,prominence=peak_height,distance=int(min_peak_distance/dw)+1)
    ...
    def get_modes_parameters(self, min_peak_level,
        ...
        for central_wavelength in mode_wavelengths:
            ...
            for jj,_ in enumerate(x_array):
                try:
                    peakind,_=find_peaks(abs(signal[:,jj]-np.nanmean(signal[:,jj])),prominence=min_peak_level , distance=int(min_peak_distance/dw)+1)
...
def get_complex_Fano_fit(waves,signal,peak_wavelength=None,height=None):
    ...
    if peak_wavelength is None:
        indexes=find_peaks(signal_abs-transmission,prominence=height)
