...
for c in win.descendants():
    ...
    if nm == "Connect" and ct in ["Text", "Button", "MenuItem"]:
        print(f"Found Connect: {ct} - clicking...")
        c.click_input()
        time.sleep(3)
        break
...
for c in win.descendants():
    ...
    if nm or tx:
        if ct in ['Window', 'Pane', 'Button', 'Text', 'TreeItem', 'DataItem', 'ListItem', 'CheckBox', 'ComboBox', 'Edit', 'TabItem']:
